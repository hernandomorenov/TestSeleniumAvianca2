...
class TestCase4LanguageAvianca:
    ...
    @allure.title("Test 4b: Quick language cycle with ActionChains")
    @allure.description("Ciclo rápido de cambio de idiomas con ActionChains")
    @allure.severity(allure.severity_level.MINOR)
    @allure.tag("language", "quick", "actionchains")
    def test_quick_language_cycle_actionchains(self, driver, setup_test):
        """Test rápido de ciclo de idiomas con ActionChains"""
        
        home_page = HomePage(driver)
        
        logger.info("="*60)
        logger.info("CICLO RÁPIDO DE IDIOMAS CON ACTIONCHAINS")
        logger.info("="*60)
        
        # Navegar a la página
        assert home_page.navigate_to(setup_test), "No se pudo navegar"
        time.sleep(2)
        
        # Ciclo por cada idioma (sin refresh)
        languages = ["spanish", "english", "french", "portuguese"]
        
        for i, language in enumerate(languages, 1):
            logger.info(f"[{i}/4] Cambiando a {language} con ActionChains...")
            
            # Cambiar idioma con ActionChains
            success = home_page.select_language(language)
            
            if success:
                logger.info(f"  ✅ {language} seleccionado")
            else:
                logger.warning(f"  ⚠️ {language} no se pudo seleccionar")
            
            # Pausa corta
            time.sleep(1)
            
            # Screenshot
            home_page.take_screenshot(f"quick_cycle_{language}")
        
        logger.info("="*60)
        logger.info("✅ Ciclo rápido completado")
        logger.info("="*60)
        
        assert True, "Ciclo rápido de idiomas completado"
    
    @allure.title("Test 4c: Language with full demo (máxima visibilidad)")
    @allure.description("Test de idiomas con máxima visibilidad para demos")
    @allure.severity(allure.severity_level.TRIVIAL)
    @allure.tag("language", "demo", "visual")
    def test_language_full_demo(self, driver, setup_test):
        """Test de idiomas con pausas largas para demos/videos"""
        
        home_page = HomePage(driver)
        
        logger.info("="*60)
        logger.info("TEST DE IDIOMAS - MODO DEMO (MÁXIMA VISIBILIDAD)")
        logger.info("="*60)
        ...
        # Solo probar 2 idiomas para demo
        demo_languages = [
            {"code": "english", "display": "English"},
            {"code": "spanish", "display": "Español"}
        ]
        ...
        for lang in demo_languages:
            logger.info(f"🎬 DEMO: Cambiando a {lang['display']}...")
            
            # Seleccionar con ActionChains (ya tiene pausas largas)
            home_page.select_language(lang['code'])
            
            # Pausa extra para demo
            logger.info("⏸️  Pausa para visualización (3 segundos)...")
            time.sleep(3)
            
            # Verificar
            home_page.verify_language_change(lang['code'])
            
            # Screenshot
            home_page.take_screenshot(f"demo_{lang['code']}")
            
            logger.info(f"✅ Demo de {lang['display']} completado")
            
            # Pausa entre idiomas
            time.sleep(2)
        
        logger.info("="*60)
        logger.info("✅ Demo de idiomas completado")
        logger.info("="*60)
